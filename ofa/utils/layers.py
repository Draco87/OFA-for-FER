...
class MBConvLayer(MyModule):
    def __init__(
        ...
        if self.mid_channels is None:
            feature_dim = make_divisible(round(self.in_channels * self.expand_ratio), MyNetwork.CHANNEL_DIVISIBLE)
        else:
            ...
            self.inverted_bottleneck = nn.Sequential(
                OrderedDict(
                    [
                        (
                            "conv",
                            MyConv2d(self.in_channels, feature_dim, 1, 1, 0, bias=False),
                        ),
        ...
        self.point_linear = nn.Sequential(
            OrderedDict(
                [
                    ("conv", MyConv2d(feature_dim, out_channels, 1, 1, 0, bias=False)),
